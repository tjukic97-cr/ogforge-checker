...
// Each rule can subtract points and add a human-readable issue.
function evaluate(tags, image) {
  const issues = [];
  let score = 100;
  ...
  if (!title) fail('no-title', 'high', 'No title found (og:title or <title>). Shares will show a bare URL.', 20);
  else if (title.length > 70) fail('title-long', 'low', 'Title is long and may be truncated on some platforms.', 4);

  // description
  if (!og('description') && !tags.description)
    fail('no-description', 'medium', 'No description. The card will look empty under the title.', 10);

  // image presence
  if (!og('image') && !tw('image')) {
    fail('no-image', 'high', 'No og:image. Most platforms will show no picture — the #1 cause of "broken-looking" links.', 35);
  } else if (image) {
    if (!image.reachable)
      fail('image-unreachable', 'high', `og:image is set but does not load (HTTP ${image.status || 'error'}). This is the classic post-deploy break.`, 30);
    if (image.reachable && image.width && image.height) {
      if (image.width < 600 || image.height < 315)
        fail('image-small', 'medium', `Image is ${image.width}×${image.height}. Recommended is 1200×630 — small images get cropped or look blurry.`, 12);
      const ratio = image.width / image.height;
      if (ratio < 1.7 || ratio > 2.0)
        fail('image-ratio', 'low', `Aspect ratio ${ratio.toFixed(2)}:1 is off the ideal ~1.91:1 and may be cropped oddly.`, 5);
    }
  }

  // twitter card
  if (!tw('card'))
    fail('no-twitter-card', 'medium', 'No twitter:card. X may render a small thumbnail instead of a large preview.', 10);
  ...
  // url / site name
  if (!og('url')) fail('no-url', 'low', 'No og:url — platforms guess the canonical link, sometimes wrongly.', 3);
  if (!og('site_name')) fail('no-site-name', 'low', 'No og:site_name — your brand name will not appear on the card.', 2);

  score = Math.max(0, Math.min(100, Math.round(score)));
  const grade =
    score >= 90 ? 'great' : score >= 75 ? 'good' : score >= 45 ? 'poor' : 'broken';
...
async function checkUrl(inputUrl) {
  ...
  const { score, grade, issues } = evaluate(tags, image);
