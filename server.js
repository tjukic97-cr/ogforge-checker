...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  button{background:linear-gradient(135deg,var(--accent2),var(--accent));color:#04141A;font-weight:700;border:none;border-radius:10px;padding:13px 22px;font-size:15px;cursor:pointer;display:inline-flex;align-items:center;gap:8px;justify-content:center;transition:opacity .2s}
  button:disabled{opacity:.7;cursor:wait}
  .spinner{width:15px;height:15px;border:2px solid rgba(4,20,26,.35);border-top-color:#04141A;border-radius:50%;animation:spin .6s linear infinite;display:inline-block}
  @keyframes spin{to{transform:rotate(360deg)}}
  /* entrance animations */
  @keyframes riseIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
  .anim{animation:riseIn .45s cubic-bezier(.16,.8,.3,1) both}
  #out{margin-top:26px}
  ...
  .pcard .platform{display:flex;align-items:center;gap:7px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);padding:11px 12px 9px;font-weight:600}
  .pcard .platform svg{flex-shrink:0;opacity:.95}
  .pcard.thumb .platform{padding:0 0 5px}
  ...
  @media(prefers-reduced-motion:reduce){.anim{animation:none}.spinner{animation-duration:1.2s}}
...
async function run(){
  ...
  if(!url){return}
  btn.disabled=true; btn.innerHTML='<span class="spinner"></span> Checking…'; out.innerHTML='';
  try{
    var res=await fetch('/api/check?url='+encodeURIComponent(url));
    var r=await res.json();
    render(r);
  }catch(e){ out.innerHTML='<p class="err anim">Something went wrong. Try again.</p>'; }
...
function previewCard(key, p, thumb, delay){
  if(!p) return '';
  var anim=' anim';
  var st=' style="animation-delay:'+(delay||0).toFixed(2)+'s"';
  if(thumb){
    return '<div class="pcard thumb'+anim+'"'+st+'><div class="pimg">'+(p.image?'<img src="'+esc(p.image)+'" alt="">':'')+'</div>'
      +'<div class="pmeta">'+platformBadge(key)+'<div class="ptitle">'+esc(p.title||'(no title)')+'</div>'
      +'<div class="pdomain">'+esc(p.domain||'')+'</div></div></div>';
  }
  return '<div class="pcard'+anim+'"'+st+'>'+platformBadge(key)+imgBlock(p)
...
function render(r){
  ...
  if(r.grade==='blocked'){
    var bhtml='<div class="score anim" style="animation-delay:0s"><div class="badge blocked">🚧</div>'
      +'<div><div class="grade">Blocked</div><div>The site wouldn\\'t let us in</div></div></div>';
    (r.issues||[]).forEach(function(i,idx){
      bhtml+='<div class="issue info anim" style="animation-delay:'+(0.1+idx*0.08)+'s"><div class="sev">heads up</div>'+i.message+'</div>';
    });
    out.innerHTML=bhtml;
    return;
  }
  ...
  var html='<div class="score anim" style="animation-delay:0s"><div class="badge" id="scoreBadge" style="border-color:'+color+';color:'+color+'">0</div>'
  ...
  if(r.previews){
    ...
    html+='<div class="preview-section anim" style="animation-delay:'+delay+'s"><p class="preview-label">How it will look when shared</p><div class="cards">';
    var cards=[['twitter',pv.twitter,twThumb],['linkedin',pv.linkedin,false],['slack',pv.slack,false],['facebook',pv.facebook,false]];
    cards.forEach(function(c){
      delay+=0.07;
      html+=previewCard(c[0],c[1],c[2],delay);
    });
    html+='</div></div>';
  }
  ...
  (r.issues||[]).forEach(function(i){
    delay+=0.07;
    html+='<div class="issue '+i.severity+' anim" style="animation-delay:'+delay.toFixed(2)+'s"><div class="sev">'+i.severity+'</div>'+i.message+'</div>';
  });
  if((r.issues||[]).length===0){ delay+=0.07; html+='<div class="issue low anim" style="animation-delay:'+delay.toFixed(2)+'s"><div class="sev">perfect</div>No issues found — this link will look great when shared.</div>'; }
  out.innerHTML=html;

  countUp(document.getElementById('scoreBadge'), r.score);
}
