...
// Headers that make our request look like a real Chrome browser, which gets us
// past the simpler bot-protection layers that would otherwise return 403.
...
function readImageMeta(imageUrl, baseUrl) {
  return new Promise((resolve) => {
    ...
    const req = lib.request(
      parsed,
      { method: 'GET', headers: { 'User-Agent': USER_AGENT, Accept: 'image/*,*/*;q=0.8' } },
      (res) => {
        const status = res.statusCode || 0;
...
async function checkUrl(inputUrl) {
  ...
  // Some sites sit behind bot-protection and refuse automated checks even
  // though the page is perfectly fine. Report that honestly (and lightly),
  // rather than scaring the user with a "BROKEN, 0" verdict.
  const BLOCKED_CODES = [401, 403, 429, 503];
  if (BLOCKED_CODES.includes(page.status)) {
    return {
      url, finalUrl: page.finalUrl, fetchedAt, ok: false, score: null, grade: 'blocked',
      tags: {}, image: null,
      issues: [{
        id: 'blocked',
        severity: 'info',
        message: `We couldn't peek at this one — looks like the site's bouncer turned us away (HTTP ${page.status}). Some sites gatekeep automated checks like this. Your link is probably fine; you'll just need to eyeball this one yourself.`,
      }],
      previews: null,
    };
  }
  ...
  if (page.status >= 400) {
    return {
      ...
      tags: {}, image: null,
      issues: [{ id: 'http-error', severity: 'high', message: `Page returned HTTP ${page.status} — the page itself isn't loading.` }],
      previews: null,
    };
  }
